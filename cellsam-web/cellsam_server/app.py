...
FINEBIO_ADAPTER_PATH = '/workspace/LeeJeongmin-project/vlm/outputs/finebio'
...
# projector 별도 로딩 — LoRA adapter만으로는 projector 가중치가 반영 안 됨
projector_path = os.path.join(FINEBIO_ADAPTER_PATH, "projector_fp32.bin")
if os.path.exists(projector_path):
    projector_state = torch.load(projector_path, map_location="cpu")
    missing, unexpected = vlm_model.load_state_dict(projector_state, strict=False)
    print(f"[DEBUG] Projector loaded. missing: {len(missing)}, unexpected: {len(unexpected)}", flush=True)
